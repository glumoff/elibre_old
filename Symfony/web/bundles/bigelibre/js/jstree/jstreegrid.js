...
//        width -= borPadWidth;
      ...
      if (!bound && resizable) {
        bound = true;
        $(document).on("selectstart", ".jstree-grid-separator", function() {
          return false;
        });
    ...
    this._prepare_grid = function(obj) {
      ...
      if (a.length === 1) {
        ...
        for (i = 1; i < cols.length; i++) {
          ...
          if (s === "attr") {
            val = col.value && objData.original.attr[col.value] ? objData.original.attr[col.value] : "";
          } else if (s === "metadata") {
            val = col.value && t.data(col.value) ? t.data(col.value) : "";
          } else if (s === "raw") {
            val = col.value ? col.value : "";
          } else if (s === "attr2") {
            var v = eval("objData.original." + col.value);
            val = col.value && v ? v : "";
          } else if (s === "callback") {
//            var v = eval("objData.original." + col.value);
//            val = col.func(v);
            val = col.func(objData.original[col.value]);
          }
